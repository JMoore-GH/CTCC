...
import type { CTCCSchema } from '../../rayfin/data/schema';
...
let client: RayfinClient<CTCCSchema> | null = null;
let localDev = false;

export function initRayfinClient(
  config: RayfinClientConfig
): RayfinClient<CTCCSchema> {
  if (client) {
    throw new Error('Rayfin client is already initialized.');
  }
  client = new RayfinClient<CTCCSchema>({
    baseUrl: config.baseUrl,
    publishableKey: config.publishableKey,
    useProxy: !config.localDev,
    authStorage: true,
  });
...
export function getRayfinClient(): RayfinClient<CTCCSchema> {
  if (!client) {
    throw new Error(
